...
public class GrappleScript : MonoBehaviour
{
    ...
    [SerializeField] private Color      chargeRingColor = Color.green;
    ...
    private Vector2    _swingLocalOffset;
    ...
    private void HandleInput()
    {
            ...
            (state == GrappleState.Idle || state == GrappleState.Retracting))
        {
            ...
            if (currentHook    != null) { Destroy(currentHook); currentHook = null; }
            _blinkTimer = 0f;
            if (chargeBarImage != null) { chargeBarImage.fillAmount = 0f; chargeBarImage.color = chargeRingColor; }
    ...
    private void UpdateCharging()
    {
        ...
        float fill = chargeTimer / maxChargeTime;

        if (chargeBarImage != null)
        {
            chargeBarImage.fillAmount = fill;

            if (fill >= 1f)
            {
                _blinkTimer += Time.deltaTime;
                bool visible = (_blinkTimer % blinkInterval) < (blinkInterval * 0.5f);
                chargeBarImage.color = visible ? chargeRingColor : Color.clear;
            }
            else
            {
                chargeBarImage.color = chargeRingColor;
            }
        }
    }
    ...
    private void UpdateLaunching()
    {
        ...
        if (hit.collider != null)
        {
            ...
            bool esHookable    = ((1 << hit.collider.gameObject.layer) & hookMask) != 0
                                 && (hit.collider.CompareTag("Hookable") || hit.collider.CompareTag("Key"));

            if      (esGrappleable) { SnapAndAttach(hit.point, hit.collider); return; }
        ...
        if (snap != null) { SnapAndAttach(snap.ClosestPoint(hookCurrentPos), snap); return; }
    ...
    private void SnapAndAttach(Vector2 punto, Collider2D hitCol = null)
    {
        ...
        // Guardar offset local para seguir objetos móviles
        _swingTarget      = hitCol;
        _swingLocalOffset = hitCol != null
            ? (Vector2)hitCol.transform.InverseTransformPoint(punto)
            : Vector2.zero;
    ...
    {
        // Si el punto de anclaje es un objeto móvil, actualizar posición y joint cada frame
        if (_swingTarget != null)
        {
            hookCurrentPos        = _swingTarget.transform.TransformPoint(_swingLocalOffset);
            joint.connectedAnchor = hookCurrentPos;
        }
    ...
    // Reseteo inmediato a Idle — llamado por KeyItem al recoger la llave
    ...
    private void UpdateHookVisual()
    {
        ...
        // Cuando está enganchado, seguir el punto real (sigue objetos móviles y Pull)
        Vector2 visualPos = state == GrappleState.Attached ? RopeEndpoint() : hookCurrentPos;
        currentHook.transform.position = visualPos;

        Vector2 dir = state == GrappleState.Launching
            ? grapplePoint - hookCurrentPos
            : (Vector2)transform.position - visualPos;
