...
public class OneWayPlatform : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        // Guard 1: player subiendo → siempre atravesar.
        // Evita que la zona de tolerancia (pies cerca del tope) solidifique y trabque al saltar desde abajo.
        float vy = playerRb != null ? playerRb.linearVelocityY : 0f;
        if (vy > 1f)
        {
        ...
        // Guard 2: centro del player fuera del ancho de la plataforma → traversal lateral (aire y suelo).
        float px = playerCol.bounds.center.x;
        if (px < col.bounds.min.x || px > col.bounds.max.x)
        {
            Physics2D.IgnoreCollision(playerCol, col, true);
            return;
        }

        // Guard 3: player grounded en otra superficie → viene de costado → atravesar.
        bool grounded = playerMovement != null && playerMovement.IsGrounded;
        if (grounded && !playerCol.IsTouching(col))
        {
            Physics2D.IgnoreCollision(playerCol, col, true);
            return;
        }

        // Lógica original: cayendo o quieto, determinar si está sobre el tope o debajo.
